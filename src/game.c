...
int main(int argc, char * argv[])
{
	...
	Sprite *speedBoost; // speed power up
	...
	tester.scaleCenter = vector2d(1, 1);
	...
	circle.scaleCenter = vector2d(1, 1);
	...
	for (int i = 0; i < 5; i++) {
		...
		slaser[i].scaleCenter = vector2d(100, 100);
	...
	sBoost.frame = 0;
	sBoost.scale = vector2d(1, 1);
	sBoost.scaleCenter = vector2d(100, 100);
	...
	while (!done)
	{
		...
		{
			slaser[count].position = vector2d(mx + 50, my + 25);
			count++;
			if (count > 4)
			{
				count = 0;
		...
		if (collide(&player, &sBoost) == true)
		{
			...
			gf2d_entity_free(&sBoost);

		}

		if (collide(&slaser, &tester) == true)
		{
